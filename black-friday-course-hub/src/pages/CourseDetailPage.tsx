...
import { coursesAPI, subscriptionsAPI, validatePromoCode, calculateDiscountedPrice, Course } from "@/data/mockData";
...
const CourseDetailPage = () => {
  ...
  const { isSubscribed, subscribe, user, refreshSubscriptions } = useAuth();
  ...
  useEffect(() => {
    const fetchCourse = async () => {
      try {
        if (id) {
          const fetchedCourse = await coursesAPI.getCourseById(id);
          setCourse(fetchedCourse);
          console.log('✅ Course loaded:', fetchedCourse.title);
        }
      } catch (error) {
  ...
  const handleSubscribe = async () => {
    ...
    setIsLoading(true);
    try {
      await subscriptionsAPI.subscribeToCourse(course!.id, promoApplied ? promoCode : undefined);
